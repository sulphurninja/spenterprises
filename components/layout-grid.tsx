...
export const LayoutGrid = ({ cards }: { cards: Card[] }) => {
  ...
  return (
    <div className="w-full h-full p-10 grid grid-cols-1 md:grid-cols-2  max-w-7xl mx-auto gap-4 relative">
      {cards.map((card, i) => (
        <div key={i} className={cn(card.className, "md:h-80")}>
          <motion.div
            ...
            className={cn(
              ...
              selected?.id === card.id
                ? "rounded-lg cursor-pointer absolute inset-0 h-1/2 w-full md:w-[70%]  m-auto z-50 flex justify-center items-center flex-wrap flex-col"
